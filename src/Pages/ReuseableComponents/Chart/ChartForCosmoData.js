...
class ChartForCosmoData extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            lineChartData: {
    ...
    handleDataInInterval(oldDataReceived) {
        ...
        var testFilterDataX = oldDataReceived.map(({ value, time }) => {
            var microtimestamp = new Date(time);
        ...
        var testFilterDataY = oldDataReceived.map(({ data, timestamp }) => {
            return data
        })
    ...
    componentDidMount() {
        ...
        //     counter++
        // }, 10000);
        this.handleDataInInterval(this.props.oldData)
...
export default withStyles(styles, { withTheme: true })(ChartForCosmoData);