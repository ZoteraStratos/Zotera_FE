...
const ListOfFileModal = (props) => {
  ...
  const handleChange = (event) => {
    const url =
      "https://wastewatertreatmentarrowdemoserver.azurewebsites.net/getListOfFile";
    fetch(url)
      .then((res) => res.text())
      .then((res) => {
        let pathNamesArray = [];
      ...
      .catch((error) => {
        ...
        setMsg(
          `Please refresh page and try one more time .There has been a problem with your fetch operation:${error}`
        );
      });
  };
    ...
    if (!files)
      items.push(
        (files = {
          ...
          files: [],
          icon: <Folder />,
          filePath: tail.length === 0 ? pathNames[indx] : "",
  ...
  useEffect(() => {
    if (selectedFilePath.length > 0) {
      ...
      if (finalFilePath.charAt(0) === "/")
        finalFilePath = finalFilePath.substr(1);
      fetch(
        `https://wastewatertreatmentarrowdemoserver.azurewebsites.net/list/filePathName?${finalFilePath}`
      )
        .then((response) => response.json())
        .then((responseJson) => {
          receiveOldData(responseJson);
          setMsg(
            " Stage 2 / 3 File content receive successfully now processing its data for chart . This window will close automatically Please wait for sometime "
          );
        ...
        .catch((error) => {
          setMsg(
            "Please refresh page and try one more time .There has been a problem with your fetch operation:",
            error
          );
          console.error(
            "Please refresh page and try one more time .There has been a problem with your fetch operation:",
              ...
              >
                <div style={getModalStyle()} className={classes.paper}>
                  ...
                  {selectedFilePath.length > 0 ? (
                    <Loader>
                      File Path you have selected is :- <br />{" "}
                      <b>{selectedFilePath}</b> <br />
                      <br />
                      <span className="blink_me">{msg}</span>
                    </Loader>
                  ) : menuData.length > 0 ? (
                    <Tree
                      data={menuData}
